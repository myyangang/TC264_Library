...
void initMotors(){
    ...
    */
    ...
    // 初始化PID
    __initPID(&velPIDl, 60, 0, 10, 0, 30);
    __initPID(&velPIDr, 60, 0, 10, 0, 30);
    __initPID(&velPIDy, 25, 0, 30, 0, 200);
    ...
    __initPID(&angPIDz, 1800, 0, 0, 0, 1000);
    
    __initPID(&angVelPIDx, 105, 4, 0, 0, 1000); //纯PI，理论上能在某个位置立住几秒，但是收积分影响，调试时需要按Reset复位积分值
    __initPID(&angVelPIDy, 10, 6, 0, 0, 1000);
    __initPID(&angVelPIDz, 10, 0, 0, 0, 1000);
    ...
    pwm_init(WHEEL_3_PWM_PIN, motorBottom.freq, 0);
...
void updateMotors(
    ...
    motorBottomSpeed = minValue(1000, motorBottomSpeed);
    motorBottomSpeed = maxValue(-1000, motorBottomSpeed);
    ...
    */
    ...
    angPIDx.target = +3.7f + (float)(-velPIDl.deltaOutput + velPIDr.deltaOutput) / 100 + (float)(angPIDz.error[0]) / 20; angPIDx.measurement = rollX; __updatePID(&angPIDx); // 手动修正误差
    angPIDy.target = -2.2f + (float)(-velPIDy.deltaOutput                      ) / 100; angPIDy.measurement = pitchY; __updatePID(&angPIDy);
    // angPIDz.target = yawTargetOffset + (float)(velPIDl.deltaOutput + velPIDr.deltaOutput) / 70;  angPIDz.measurement = yawZ; __updatePID(&angPIDz);
    angPIDz.target = yawTargetOffset;  angPIDz.measurement = yawZ; __updatePID(&angPIDz);
    ...
    /* 通过角度环输出, 决定角速度环target
        ...
        分析:
            ...
            Y: 当车身pitch<0时,measurement<0. 车身已有角动量=(0,+,0). 此时target - measurement更可能为正, 即angPIDy输出更可能为正.
                我们期望角动量(0,+,0)转移到底部轮上,也就是说底轮(0,+,0)↑
    ...
    /* 通过角速度环输出,决定PWM
        ...
        分析:
            ...
            Y: 当pitch↑时,角动量为(0,-,0),gyroZ->angVelY为正. 此时measurement↑,angvelPIDy输出值target-measurement减小
                我们期望底轮pwm↓,角动量(0,-,0),反作用角动量(0,+,0),可以抵消Y方向角动量
                ∴pwmB += angvelPIDy
