										 ...
										 uint16 *dir_l, uint16 *dir_r, uint16(*points_l)[2], uint16(*points_r)[2])
{
	...
	if (break_num_l&&break_num_r&&image[image_h - 1][4] && image[image_h - 1][image_w - 4])//两边生长方向都符合条件
	{
		...
		start = limitRange(start, 0, image_h);
		end = break_num_l - 5;
		...
		for (i = break_num_l - 5; i < image_h - 1; i++)
		{
			...
			l_border[i] = limitRange(l_border[i], border_min, border_max);//限幅
		}

		//计算斜率
		start = break_num_r - 15;//起点
		start = limitRange(start, 0, image_h);//限幅
		...
		for (i = break_num_r - 5; i < image_h - 1; i++)
		{
			...
			r_border[i] = limitRange(r_border[i], border_min, border_max);
		}
